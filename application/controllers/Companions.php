...
class Companions extends Admin_Controller
{
    ...
    public function fetchCompData()
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (in_array('updateCompanion', $this->permission)) {
                ...
                $buttons .= '<a href="' . base_url('companions/update/' . $value['id']) . '" class="btn btn-default"><i class="fa fa-eye"></i></a>';
            }
    ...
    public function fetchItemsData($comp_id)
    {
        ...
        if ($this->model_companions->existInCompanions($comp_id) == FALSE) {
            ...
            $this->session->set_flashdata('error', 'Společník vám nepatří!');
            redirect('companions', 'refresh');
        } else {
            $result = array('data' => array());
    ...
    public function update($comp_id)
    {
        ...
        if ($this->model_companions->existInCompanions($comp_id)) {
            if ($this->model_companions->userIdMatch($this->session->userdata('id'), $comp_id)) {
                $this->render_template('companions/update', $this->data);
            } else {
                $this->session->set_flashdata('error', 'Společník vám nepatří!');
                redirect('companions', 'refresh');
            }
    ...
    public function delete()
    {
        ...
        if ($comp_id) {
            if ($this->model_companions->existInCompanions($comp_id) == FALSE) {
                ...
                $delete = $this->model_companions->delete($comp_id);
                if ($delete == true) {
                    $response['success'] = true;
